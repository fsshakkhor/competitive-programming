...
{
    ll xx = x;
    if ((double)xx==x) return true;
    return false;
}

vector<pair<PT, PT>> v;
ll x1[1007], x2[1007], y1[1007], y2[1007];
...
int main ()
{
    ...
    for (i=0; i<n; i++)
    {
        ...
        PT p(x1[i], y1[i]), q(x2[i], y2[i]);
        v.push_back(make_pair(p, q));
        ll g = __gcd(abs(x1[i]-x2[i]), abs(y1[i]-y2[i]));
        cnt += g+1;
    }
    ...
    for (i=0; i<n; i++)
    {
        for (j=i+1; j<n; j++)
        {
            if (SegmentsIntersect(v[i].first, v[i].second, v[j].first, v[j].second))
            {
                PT now = ComputeLineIntersection(v[i].first, v[i].second, v[j].first, v[j].second);
//                cout << now << endl;
                if (check(now.x) && check(now.y))
                {
                    ll xx = now.x, yy = now.y;
                    mp[make_pair(xx, yy)]++;
                }
            }
        }
